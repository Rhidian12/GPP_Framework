...
void App_AgarioGame_BT::Start()
{
	...
	for (int i = 0; i < m_AmountOfAgents; i++)
	{
		...
		BehaviorTree* pBehaviorTree{ new BehaviorTree{pBlackboard, new BehaviorAction{ChangeToWander}} };
	...
	const float radiusToCheckForLargerEnemies{ 20.f };
	const float radiusToCheckForSmallerEnemies{ 20.f };
	AgarioAgent* pFleeTarget{};
	AgarioAgent* pChaseTarget{};
	const float minimumRadiusForSmartAgent{ 5.f };

	Blackboard* pBlackboard{ CreateBlackboard(m_pUberAgent) };
	pBlackboard->AddData("smartAgent", m_pUberAgent);
	pBlackboard->AddData("dumbAgents", &m_pAgentVec); // == Reference The Vector To Avoid A Copy ==
	pBlackboard->AddData("radiusToCheckForLargerEnemies", radiusToCheckForLargerEnemies);
	pBlackboard->AddData("fleeTarget", pFleeTarget);
	pBlackboard->AddData("chaseTarget", pChaseTarget);
	pBlackboard->AddData("minimumRadius", minimumRadiusForSmartAgent);
	...
	BehaviorTree* pBehaviourTree
	{
		...
		{pBlackboard,
			...
			{{
				...
				{
					new BehaviorConditional{IsLargerEnemyClose},
				...
				new BehaviorSelector{
				{
					new BehaviorSequence{
						{
							new BehaviorConditional{IsMinRadiusReached},
							new BehaviorConditional{IsSmallerEnemyClose},
							new BehaviorAction{ChaseSmallerEnemy}
						}},
					new BehaviorSequence{
						{
							//new BehaviorConditional{IsMinRadiusReached},
...
void App_AgarioGame_BT::Render(float deltaTime) const
{
	...
	DEBUGRENDERER2D->DrawCircle(m_pUberAgent->GetPosition(), 20.f, Elite::Color{ 1.f,0.f,0.f }, 0.4f);
