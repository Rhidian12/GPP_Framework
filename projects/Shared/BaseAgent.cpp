...
void BaseAgent::TrimToWorld(const Elite::Rect& bounds) const
{
	auto pos = GetPosition();

	if (pos.x > bounds.bottomLeft.x + bounds.width)
		pos.x = bounds.bottomLeft.x;
	else if (pos.x < bounds.bottomLeft.x)
		pos.x = bounds.bottomLeft.x + bounds.width;

	if (pos.y > bounds.bottomLeft.y + bounds.height)
		pos.y = bounds.bottomLeft.y;
	else if (pos.y < bounds.bottomLeft.y)
		pos.y = bounds.bottomLeft.y + bounds.height;
