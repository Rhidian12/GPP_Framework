...
namespace Elite
{
	...
	void InfluenceMap<T_GraphType>::PropagateInfluence(float deltaTime)
	{
		...
		if (m_TimeSinceLastPropagation >= m_PropagationInterval)
		{
			...
			{
				float influence{ 0.f };
				for (auto& pNodeConnection : GetNodeConnections(pNode))
				{
					float newInfluence = GetNode(pNodeConnection->GetTo())->GetInfluence() * expf(-pNodeConnection->GetCost() * m_Decay);
					if (abs(newInfluence) > abs(influence))
						influence = newInfluence;
				}
				m_InfluenceDoubleBuffer.push_back(Lerp(pNode->GetInfluence(), influence, m_Momentum));
			}
			int index{};
			for (auto& pNode : GetAllActiveNodes())
			{
				pNode->SetInfluence(m_InfluenceDoubleBuffer[index]);
				++index;
			}
